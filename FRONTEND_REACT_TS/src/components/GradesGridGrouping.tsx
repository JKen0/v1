...
const groupDataByTerm = (
  data: GridDataInterface[],
): GroupingGridDataInterface[] => {
  const groupedData: GroupingGridDataInterface[] = [];

  // Group data by term
  const groupedByTerm = data.reduce(
    (acc: { [term: string]: GridDataInterface[] }, curr: GridDataInterface) => {
      if (acc[curr.Term]) {
  ...
  return (
    <TableHead>
      <TableRow>
        ...
        <TableCell style={{ fontSize: 'large' }} padding={'normal'}>
          Course Code
        </TableCell>
        <TableCell style={{ fontSize: 'large' }} padding={'normal'}>
          Course Name
        ...
        <TableCell style={{ fontSize: 'large' }} padding={'normal'}>
          ...
          <IconButton onClick={setModalOpen}>
            <InfoIcon fontSize="small" style={{ marginTop: '-3px' }} />
          </IconButton>
        </TableCell>
        <TableCell style={{ fontSize: 'large' }} padding={'normal'}>
          Units
